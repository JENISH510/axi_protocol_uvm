...
class axi_m_mon#(int ADDR_WIDTH=32, DATA_WIDTH=32) extends uvm_monitor;
	...
	task write_addr_channel();
      ...
      `uvm_info("M MON write addr PHASE after","Inside master mon", UVM_MEDIUM)
  	endtask
              ...
              @(m_vif.m_mon_cb iff (m_vif.m_mon_cb.WVALID && m_vif.m_mon_cb.WREADY));
                ...
                if(get_id.exists(wid))begin
                    ...
                    if (m_vif.m_mon_cb.WLAST) begin
        		        m_port.write(w_trans);
        		        void'(awid_q.pop_front());
                        get_id.delete(wid);
      			    end
                end
    		end
      `uvm_info("M MON write data PHASE after","Inside master mon", UVM_MEDIUM)
  	endtask
	...
	task read_addr_channel();
      ...
      `uvm_info("M MON read addr PHASE before","Inside master mon", UVM_MEDIUM)
    		forever begin
              ...
              //$display("arid in m mon :%0d",ar_trans.ARID);
                ...
                //arid_q.push_back(ar_trans.ARID);
                $display("m mon ar id : %0h", ar_trans.ARID);
    		end
      `uvm_info("M MON read addr PHASE after","Inside master mon", UVM_MEDIUM)
  	endtask
	...
	task read_data_channel();
        ...
        `uvm_info("M MON read data PHASE after","Inside master mon", UVM_MEDIUM)
    endtask
